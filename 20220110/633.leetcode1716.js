 ...
 */
 var totalMoney = function(n) {
  if (n < 8) return new Array(n).fill(0).reduce((pre, total, index) => total = pre + (index + 1),0)
  const firstWeek = 1 + 2 + 3 + 4 + 5 + 6 + 7
  const count = Math.floor(n / 7), left = n % 7
  let ans = count * firstWeek

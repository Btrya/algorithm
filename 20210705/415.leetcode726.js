 ...
 */
 var countOfAtoms = function(formula) {
  ...
  while (i < n) {
    ...
    if (ch === '(') {
      i++
      stack.unshift(new Map()) // 如果匹配到左括号 将一个空的哈希表压入栈
    } else if (ch === ')') {
      i++
      ...
      for (const [atom, v] of popMap.entries()) {
        topMap.set(atom, (topMap.get(atom) || 0) + v * num) // 括号内的原子数量乘上num,加到上一层的原子数量中
      }
    } else {

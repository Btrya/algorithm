 ...
 function isValid(code: string): boolean {
  ...
  while (i < n) {
    ...
    if (code[i] === '<') {
      ...
      // 是末尾的尖角，表达这个标签的结束
      if (code[i + 1] === '/') {
        // 找到下一个右尖角，找不到的话就是不合法的，找到了就能知道 tag_name
        const j = code.indexOf('>', i)
        if (j < 0) return false
        const tagname = code.slice(i + 2, j)
        // 没有匹配的左侧 tag_name 不合法
        if (tags.length === 0 || tags[tags.length - 1] !== tagname) return false
        tags.pop()
        // 跳到 右尖角的下一位
        i = j + 1
        // 没有左侧标签了，但后续还有字符串
        if (tags.length === 0 && i !== n) return false
      } else if (code[i + 1] === '!') {
        // 遇到了 cdata 判断是否是 [CDATA[ 开头， 直接定位到下一个 ]]> 就可以得到中间的内容
        // cdata 必须在标签内 也就是说 必须要有左侧标签这时候，否则就是不合法
        if (tags.length === 0) return false
        // 越界 遇到了 ! 那么他的后边应该有 [CDATA[ 注意这里 i 是 < 的位置，所以 +9 包括<![CDATA[
        ...
        for (let k = 0; k < tagname.length; ++k) {
          if (!(tagname[k] >= 'A' && tagname[k] <= 'Z')) return false
        }
        tags.push(tagname)
        i = j + 1
      }
